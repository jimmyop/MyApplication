...
import com.jimmy.commonlibrary.widget.loadingmanager.LoadingManager;
import com.jimmy.commonlibrary.widget.loadingmanager.LoadingUiType;
...
public class RequestUiManager {
     ...
     */
    protected LinkedHashMap<Object, GsonRequest> requestsMap = new LinkedHashMap<>();
    /**
     * 存放请求的回调对象  key -- request getTag
     */
    protected HashMap<Object, HttpCallBack> callbackListenersMap = new HashMap<>();
    ...
    public LinkedHashMap<Object, GsonRequest> getRequestsMap() {
        return requestsMap;
    }

    public void setRequestsMap(LinkedHashMap<Object, GsonRequest> requestsMap) {
        this.requestsMap = requestsMap;
    }

    public HashMap<Object, HttpCallBack> getCallbackListenersMap() {
        return callbackListenersMap;
    }

    public void setCallbackListenersMap(
            HashMap<Object, HttpCallBack> callbackListenersMap) {
     ...
     */
    public void addRequestAndCallbackListener(GsonRequest request, HttpCallBack callbackListener) {
//        callbackListenersMap.put(request.getRequestTag(), callbackListener);
//        requestsMap.put(request.getRequestTag(), request);
    }

    protected void removeRequestAndCallbackListener(GsonRequest request) {
        callbackListenersMap.remove(request.getTag());
        requestsMap.remove(request.getTag());
    }

    public HttpCallBack getRequestCallBackListener(GsonRequest request) {
        HttpCallBack responseCallbackListener = callbackListenersMap.get(request.getTag());
